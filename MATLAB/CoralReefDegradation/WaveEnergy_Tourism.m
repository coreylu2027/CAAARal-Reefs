...
if exist('years_all','var')        && exist('annual_mean_hc','var') && ...
   exist('future_years_vec','var') && exist('yF','var')
    ...
    fcst_years = 2024:2029;
    fcst_cover = max(1, obs_cover(end) - 0.55*(1:6));
...
% Uncertainty bounds
D_wave_lo = D_max .* (1 - exp(-lambda_dmg .* max(0,(E_shore(fcst_lo)-E_ref_val)/E_ref_val)));
D_wave_hi = D_max .* (1 - exp(-lambda_dmg .* max(0,(E_shore(fcst_hi)-E_ref_val)/E_ref_val)));
dM_lo     = max(0, M_base - T0.*(fcst_hi/A0).*M0);
dM_hi     = max(0, M_base - T0.*(fcst_lo/A0).*M0);
L_lo      = D_wave_lo + dM_lo;
L_hi      = D_wave_hi + dM_hi;
...
fill([fcst_years, fliplr(fcst_years)], [fcst_hi, fliplr(fcst_lo)], ...
    C_CI,'EdgeColor','none','FaceAlpha',1);
...
lg5 = legend(ax10,'Location','northwest');
