...
% Load all benthic cover files from the raw data folder.
% The recursive glob picks up files across region subfolders.
pwd
...
for f = 1:length(files)
    ...
    try
        ...
        % Some files have a units row right under the header - skip it
        if height(tbl) > 0 && isnumeric(tbl{1,1})
            tbl(1,:) = [];
        end
        ...
        if isempty(data_all)
            ...
            % Files from different regions often have slightly different columns.
            % Rather than crashing on a straight vertical cat, we pad missing
            % columns with NaN so everything lines up.
            missing_in_new = setdiff(data_all.Properties.VariableNames, tbl.Properties.VariableNames);
            for mc = 1:length(missing_in_new)
                tbl.(missing_in_new{mc}) = repmat({NaN}, height(tbl), 1);
            end
            missing_in_old = setdiff(tbl.Properties.VariableNames, data_all.Properties.VariableNames);
            for mc = 1:length(missing_in_old)
                data_all.(missing_in_old{mc}) = repmat({NaN}, height(data_all), 1);
            end
            data_all = [data_all; tbl(:, data_all.Properties.VariableNames)];
        end
...
% Hard coral species codes used by CRCP/AGRRA taxonomy
hard_coral_codes = {
...
% Convert columns that should be numeric but may have come in as cell/string
...
for nc = 1:length(num_cols)
    col = num_cols{nc};
    if ismember(col, col_names) && iscell(data_all.(col))
        data_all.(col) = cellfun(@(x) str2double(string(x)), data_all.(col));
    end
end

% Build a unique transect identifier from year + PSU + station.
% PRIMARY_SAMPLE_UNIT is alphanumeric so we treat everything as strings.
if ismember('PRIMARY_SAMPLE_UNIT', col_names)
    psu_col = string(data_all.PRIMARY_SAMPLE_UNIT);
else
    psu_col = repmat("0", height(data_all), 1);
end

if ismember('STATION_NR', col_names)
    stn_col = string(data_all.STATION_NR);
else
    stn_col = repmat("0", height(data_all), 1);
end

year_col = data_all.YEAR;
transect_id = string(year_col) + "_" + psu_col + "_" + stn_col;
unique_transects = unique(transect_id);
n_transects = length(unique_transects);
fprintf('\nUnique transects: %d\n', n_transects);

% Pre-allocate the transect-level summary struct.
% psu and station stay as strings because PSU codes aren't numeric.
T = struct();
T.year          = zeros(n_transects, 1);
T.psu           = strings(n_transects, 1);
T.station       = strings(n_transects, 1);
T.lat           = zeros(n_transects, 1);
T.lon           = zeros(n_transects, 1);
...
T.depth_m       = zeros(n_transects, 1);
T.rugosity      = NaN(n_transects, 1);
T.protection    = zeros(n_transects, 1);
...
cat_col = string(data_all.COVER_CAT_CD);

for i = 1:n_transects
    rows = find(transect_id == unique_transects(i));

    T.year(i)    = year_col(rows(1));
    ...
    if ismember('WTD_RUG', col_names)
        T.rugosity(i) = mean(data_all.WTD_RUG(rows), 'omitnan');
    end
    if ismember('PROT', col_names),           T.protection(i) = data_all.PROT(rows(1));               end
    if ismember('SUB_REGION_NAME', col_names), T.sub_region(i) = string(data_all.SUB_REGION_NAME(rows(1))); end
    if ismember('HABITAT_CD', col_names),      T.habitat(i)    = string(data_all.HABITAT_CD(rows(1)));      end
    if ismember('ZONE_NAME', col_names),       T.zone(i)       = string(data_all.ZONE_NAME(rows(1)));       end
...
% Quick annual summary to the console
...
% Annual means and standard errors for plotting and modeling
...
subplot(2,2,3);
c_yr = transect_tbl.year;
scatter(transect_tbl.algae, transect_tbl.hard_coral, 30, c_yr, 'filled', 'MarkerFaceAlpha', 0.5);
colormap(subplot(2,2,3), cool(4));
cb = colorbar; cb.Label.String = 'Year';
xlabel('Algae Cover (%)'); ylabel('Hard Coral Cover (%)');
title('Coral vs Algae (all transects)');
p_corr = polyfit(transect_tbl.algae, transect_tbl.hard_coral, 1);
x_range = linspace(0, max(transect_tbl.algae), 100);
hold on;
plot(x_range, polyval(p_corr, x_range), 'r--', 'LineWidth', 2);
...
% Panel regression to extract a covariate-adjusted trend.
% Predictors: year, depth (z-scored), algae (z-scored), protection status.
fprintf('\nPanel regression...\n');
...
% ARIMA on annual means.
% Only a handful of survey years exist, so AR(1), MA(1), and white noise
% are the only models that won't overfit. Results should be treated as
% rough trajectory estimates, not precise predictions.
fprintf('\nARIMA on annual time series...\n');
fprintf('  Note: few time points - AR(1)/MA(1) only, interpret carefully.\n\n');
...
% Keep the declining trend as-is. Coral loss is real and flooring it
% to zero would mask the ecological signal.
if p_trend(1) < 0
    fprintf('  Declining trend: %.4f%%/obs - retaining as-is.\n', p_trend(1));
else
...
[yF_det, yMSE] = forecast(best_fit, forecast_years, 'Y0', y_detrended);

last_t = length(y_annual);
future_t = (last_t+1):(last_t+forecast_years);
...
future_years = (last_year+1):(last_year+forecast_years);

fprintf('\n  %-6s %-10s %-10s %-10s\n', 'Year','Forecast','Lower95','Upper95');
fprintf('  %s\n', repmat('-',1,40));
for f = 1:forecast_years
    fprintf('  %-6d %-10.2f %-10.2f %-10.2f\n', future_years(f), yF(f), yF_lower(f), yF_upper(f));
end

% ARIMAX: coral ~ f(algae as exogenous regressor).
% Algae is projected forward with a simple linear trend since we don't have
% a separate algae forecast model.
...
try
    ...
    fprintf('  Best: %s\n', arimax_name);
    fprintf('  Algae coef (std): %.4f  -> 1-SD algae increase = %.4f%% coral change\n', ...
        best_arimax.Beta, best_arimax.Beta);

    t_alg = (1:length(y_algae_annual))';
    p_alg = polyfit(t_alg, y_algae_annual, 1);
    future_t_alg  = (length(y_algae_annual)+1):(length(y_algae_annual)+forecast_years);
    alg_forecast_raw = polyval(p_alg, future_t_alg');
    alg_forecast_z   = (alg_forecast_raw - mean(y_algae_annual)) / std(y_algae_annual);

    yF_arimax = forecast(best_arimax, forecast_years, 'Y0', y_annual, ...
                         'X0', X_alg, 'XF', alg_forecast_z);
    ...
    fprintf('\n  %-6s %-10s %-10s %-12s\n', 'Year','Forecast','Algae%','CoralChange');
    for f = 1:forecast_years
    ...
    fprintf('  ARIMAX failed: %s - falling back to ARIMA.\n', ME.message);
    results_arimax.success    = false;
    results_arimax.forecast   = yF;
    results_arimax.model_name = 'ARIMA (ARIMAX fallback)';
end

% Regional breakdown. Only fit models where we have at least 3 years of data.
fprintf('\nRegional analysis...\n');
...
for s = 1:length(model_regions)
    ...
    fprintf('    years: %s\n', num2str(sub_years));
    fprintf('    mean cover: %.2f%% (%.2f - %.2f)\n', mean(y_reg), min(y_reg), max(y_reg));
    ...
    try
        ...
        [yF_det_reg, yMSE_reg] = forecast(fit_reg, forecast_years, 'Y0', y_det_reg);

        ft_reg   = (length(y_reg)+1):(length(y_reg)+forecast_years);
        yF_reg   = max(0, min(100, yF_det_reg + polyval(p_reg, ft_reg')));
        yF_reg_up = min(100, max(0, yF_det_reg + polyval(p_reg, ft_reg') + 1.96*sqrt(yMSE_reg)));
        yF_reg_lo = max(0,   yF_det_reg + polyval(p_reg, ft_reg') - 1.96*sqrt(yMSE_reg));

        future_yr_reg = (sub_years(end)+1):(sub_years(end)+forecast_years);
        ...
        fprintf('    trend: %.4f%%/yr  |  2-yr forecast mean: %.2f%%\n', ...
            p_reg(1), mean(yF_reg(1:min(2,end))));
        ...
        t_all     = (1:length(yr_all))';
        trend_all = polyval(p_reg, t_all);
...
% Main Florida-wide forecast figure
figure('Position', [50 50 1400 700], 'Name', 'Florida Benthic Cover Forecast');
...
trend_x = linspace(years_all(1), future_years(end), 100);
trend_t = linspace(1, length(y_annual)+forecast_years, 100);
plot(trend_x, polyval(p_trend, trend_t), 'k--', 'LineWidth', 1.5, ...
    'DisplayName', sprintf('Trend (%.4f%%/yr)', p_trend(1)));

plot(future_years, yF, 'r-o', 'LineWidth', 2.5, 'MarkerSize', 9, ...
    'MarkerFaceColor', 'r', 'DisplayName', sprintf('ARIMA (%s)', best_model_name));
fill([future_years, fliplr(future_years)], [yF_upper', fliplr(yF_lower')], ...
    'r', 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'DisplayName', '95% CI');

if results_arimax.success
    plot(future_years, results_arimax.forecast, 'g-s', 'LineWidth', 2, 'MarkerSize', 8, ...
        'MarkerFaceColor', 'g', 'DisplayName', results_arimax.model_name);
end

xlabel('Year'); ylabel('Hard Coral Cover (%)');
title('Florida Reef Tract - Hard Coral Forecast');
legend('Location', 'best', 'FontSize', 9);
grid on;
ylim([0, max([annual_mean_hc; yF_upper])*1.3 + 1]);
xlim([years_all(1)-1, future_years(end)+0.5]);
...
if results_arimax.success
    plot(future_years, results_arimax.algae_forecast, 'g--s', 'LineWidth', 2, ...
...
plot(future_years, yF, 'r--', 'LineWidth', 2, 'DisplayName', 'Forecast Coral');
...
sgtitle('Florida Reef Tract - ARIMA + ARIMAX Forecasts');

% Top species trends - stacked composition and per-species lines
...
for sp = 1:length(top_species)
    sp_rows = strcmp(string(data_all.COVER_CAT_CD), top_species{sp});
    for y = 1:length(years_all)
...
% Residual diagnostics for both the panel regression and ARIMA
y_hat     = X_panel * b_ols;
...
fprintf('\n%s\n', repmat('-',1,72));
fprintf('Forecast summary - Florida hard coral cover\n');
fprintf('%s\n', repmat('-',1,72));
...
for f = 1:forecast_years
    if results_arimax.success
        arimax_val = results_arimax.forecast(f);
    else
        arimax_val = NaN;
    end
    fprintf('%-8d %-14.2f %-14.2f %-14.2f %-14.2f\n', ...
        future_years(f), yF(f), yF_lower(f), yF_upper(f), arimax_val);
end
...
global_forecast_tbl = table(future_years', yF, yF_lower, yF_upper, ...
    'VariableNames', {'Year','ARIMA_Forecast_HardCoral_Pct','Lower_95CI','Upper_95CI'});

if results_arimax.success
    global_forecast_tbl.ARIMAX_Forecast     = results_arimax.forecast;
    global_forecast_tbl.ARIMAX_Lower        = results_arimax.forecast_lower;
    global_forecast_tbl.ARIMAX_Upper        = results_arimax.forecast_upper;
    global_forecast_tbl.Projected_Algae_Pct = results_arimax.algae_forecast;
end

writetable(global_forecast_tbl, 'benthic_cover_forecast_florida.csv');
fprintf('  benthic_cover_forecast_florida.csv\n');
