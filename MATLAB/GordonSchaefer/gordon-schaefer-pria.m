...
K              = K_FL * (reef_area_PRIA / reef_area_FL);
X0_raw = 13186.4637; K_raw = 40989.3037;
X0     = K * (X0_raw / K_raw);

r       = 0.35;
q       = 0.005;
sigma_X = 0.40;     % capped from raw CV 0.81 (inflated by rotating survey schedule)
E       = 20;
...
g       = 0.109;
delta   = 0.047;
sigma_R = 0.20;     % capped from raw CV 0.55 (only 2 yrs data)
alpha   = K / Rmax;

p_shock         = 0.12;
reef_shock_mean = 0.30;
reef_shock_sd   = 0.10;
fish_shock_mean = 0.22;
fish_shock_sd   = 0.08;

p_fish        = 8;
discount_rate = 0.015;
scale_to_global = 284000 / reef_area_PRIA;
Global_Pop      = 8e9;
...
plot(years(1), X0, 'ro','MarkerSize',5,'MarkerFaceColor','r','DisplayName','Observed 2023')
...
ylabel('Biomass (kg)')
xlabel('Year')
title('Fish Biomass Projection — Pacific Remote Island Areas  (n = 500)')
ylim([0 25000]); xlim([2022 2103])
legend('Location','northeast')
grid on

exportgraphics(gcf,'pria_projection.png','Resolution',600)
