...
regions(3).discount_rate   = 0.03;    % Florida uses higher discount rate
...
for ri = 1:nR
    ...
    results(ri).X_p05       = prctile(X_all_deg,  2.5, 1);
    results(ri).X_p95       = prctile(X_all_deg, 97.5, 1);
    results(ri).K           = K;
...
% ---- Color palette (one per region) ----
c_red   = [0.84 0.19 0.15];
c_shade = [0.92 0.92 0.92];

reg_colors = [
    0.13 0.47 0.71;   % American Samoa
    0.55 0.34 0.65;   % CNMI
    0.17 0.63 0.43;   % Florida
    0.95 0.55 0.15;   % Hawaii
    0.84 0.19 0.15;   % PRIA
    0.10 0.60 0.75;   % Puerto Rico
    0.65 0.40 0.20;   % USVI
];

% ---- Figure 1: Comparative Fish Biomass (X/K) ----
npv_prems = [results.npv_premium];
[~, idx_high] = max(npv_prems);
[~, idx_low]  = min(npv_prems);

fig1 = figure('Color','w','Position',[40 40 1400 500]);
...
for ri = 1:nR
    is_extreme = (ri == idx_high || ri == idx_low);
    if is_extreme
        fill(axA, [years, fliplr(years)], ...
            [results(ri).X_p95./results(ri).K, fliplr(results(ri).X_p05./results(ri).K)], ...
            reg_colors(ri,:), 'FaceAlpha',0.15,'EdgeColor','none');
    end
...
xlim(axA,[2022 2107]);
ylim(axA,[0 1.15]);
ylabel(axA,'Fish Biomass  $(X / K)$','FontSize',11,'Interpreter','latex','Color',[0.15 0.15 0.15]);
xlabel(axA,'Year','FontSize',11,'Interpreter','latex','Color',[0.15 0.15 0.15]);
title(axA,'Fish Biomass Projection — All US Reef Regions  ($n = 500$)', ...
    'FontSize',13,'FontWeight','bold','Interpreter','latex','Color',[0.10 0.10 0.10]);
...
legend(axA, [hShock; hCI; hLines], ...
    ['Shock window (2026)'; '$95\%$ CI (extreme regions)'; legend_labels(:)], ...
    'FontSize',8.5,'Interpreter','latex', ...
    'EdgeColor',[0.82 0.82 0.82],'Color','w', ...
    'Location','northeast','NumColumns',2);

exportgraphics(fig1,'figure1_biomass_trajectories.pdf','ContentType','vector','BackgroundColor','white');
disp('Saved: figure1_biomass_trajectories.pdf');

% ---- Figure 2: NPV Degradation Premium Bar Chart ----
...
fig2 = figure('Color','w','Position',[60 60 1050 560]);
tiledlayout(1,1,'Padding','loose');
...
exportgraphics(fig2,'figure2_npv_premium.pdf','ContentType','vector','BackgroundColor','white');
disp('Saved: figure2_npv_premium.pdf');
